        ...
        <section class="mx-auto max-w-5xl px-4 py-10 sm:px-6 lg:px-8">
            <div class="mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
                <di>
                    ...
                    <p class="mt-2 text-sm text-slate-500 dark:text-slate-400">
                        Data analisis yang kamu hapus akan muncul di sini. Kamu bisa memulihkan data atau menghapusnya
                        permanen.
                    </p>
            </div>
            </div>
            ...
            @if ($analyses->isEmpty())
                <div
                    ...
                    <p class="mt-2 text-sm text-slate-500 dark:text-slate-400">
                        History yang kamu hapus akan muncul di halaman ini.
                    </p>
                            ...
                            <div class="flex flex-col justify-between gap-5 sm:flex-row sm:items-center">
                                <div>
                                    ...
                                    <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
                                        Dihapus pada
                                        {{ $analysis->deleted_at?->timezone('Asia/Jakarta')->format('d M Y, H:i') }} WIB
                                    </p>
                                </div>

                                <div class="flex flex-wrap gap-2">
                                    <form method="POST" action="{{ route('trash.restore', $analysis->id) }}"
                                        class="js-restore-trash-form"
                                        data-title="{{ $analysis->target_role ?? 'Career Analysis' }}">
                                        @csrf
                                        @method('PATCH')

                                        <button type="submit"
                                            class="rounded-2xl bg-[#34A853] px-4 py-2.5 text-sm font-black text-white transition hover:bg-green-600">
                                            Restore
                                        </button>
                                    </form>

                                    <form method="POST" action="{{ route('trash.force-delete', $analysis->id) }}"
                                        class="js-force-delete-trash-form"
                                        data-title="{{ $analysis->target_role ?? 'Career Analysis' }}">
                                        @csrf
                                        @method('DELETE')
    ...
    @once
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                document.querySelectorAll('.js-restore-trash-form').forEach(function(form) {
                    form.addEventListener('submit', function(event) {
                        ...
                        Swal.fire({
                            ...
                            text: `Data "${title}" akan dikembalikan ke halaman History.`,
                            icon: 'question',
                            ...
                            customClass: {
                                ...
                                confirmButton: 'rounded-2xl bg-[#34A853] px-5 py-3 text-sm font-black text-white transition hover:bg-green-600',
                ...
                document.querySelectorAll('.js-force-delete-trash-form').forEach(function(form) {
                    form.addEventListener('submit', function(event) {
                        event.preventDefault();
                        ...
                        Swal.fire({
                            ...
                            text: `Data "${title}" akan dihapus permanen dan tidak bisa dikembalikan.`,
                            icon: 'warning',
                            showCancelButton: true,
